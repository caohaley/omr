...
TEST(PortSockTest, two_socket_datagram_communication)
{
	...
	connect_client_to_server(OMRPORTLIB, "localhost", "17300", OMRSOCK_AF_INET, OMRSOCK_DGRAM, &clientSocket, &clientSockAddr, &serverSockAddr);
	...
	*/
	...
	ASSERT_EQ(OMRPORTLIB->sock_setsockopt_timeval(OMRPORTLIB, serverSocket, OMRSOCK_SOL_SOCKET, OMRSOCK_SO_RCVTIMEO, &timeSend), 0);
	ASSERT_EQ(OMRPORTLIB->sock_setsockopt_timeval(OMRPORTLIB, clientSocket, OMRSOCK_SOL_SOCKET, OMRSOCK_SO_SNDTIMEO, &timeRecv), 0);
	...
	for (int32_t i = 0; i < 200; i++) {
		bytesRecv = OMRPORTLIB->sock_recvfrom(OMRPORTLIB, serverSocket, (uint8_t *)bufDgram, bytesTotal, 0, &clientSockAddr);
		if (bytesRecv < 0) {
			printf("%d\n", omrerror_last_error_number());
			FAIL() << "Recv error: " << ::testing::PrintToString(omrerror_last_error_number());
		}
